...
- ✅ **Fix (`server.py` studio_reports):** skip null/missing `assessment_id` and `score`; coerce all score reads with `(x or 0)`; guard `pbid` against missing `mission_id`; wrap per-student aggregation in try/except so one malformed record can never blank the whole class.
- ✅ Verified: reproduced the 500 with a seeded null-score attempt, applied fix → endpoint returns 200 with the class (uses best valid attempt), xlsx export 200; added regression `tests/test_reports_blocks.py::test_null_score_attempt_does_not_break_report` (10/10 pass). ⚠️ Production-affecting — requires redeploy to restore the live Reports view.
